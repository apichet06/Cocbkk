...
$sql = mysqli_query($conn, "SELECT c_time_1,c_time_2 ,c_quantity_1,c_quantity_2,c_date  FROM church_time WHERE c_date ='$c_date' ");
...
// echo $date_time .' == ' . $rs['c_date'] .' '. $rs['c_time_1'];
   if(strtotime($date_time) <= $date_1){
    echo '<label class="col-sm-6 col-form-label">รอบที่ 1 เวลา: '. $new_date_1 . ' เหลือที่นั่งว่าง => '; if($s1 <=0){ echo "เต็ม"; }else{
    echo $s1; } echo '</label>';
    }

   if(strtotime($date_time) <= $date_2){
echo '<label class="col-sm-6 col-form-label">รอบที่ 2 : '. $new_date_2 . ' เหลือที่นั่งว่าง => '; if($s2 <=0){ echo "เต็ม"; }else{
    echo $s2; } echo '</label>';
   }
echo '</div>';
    if ($s1 > 0 or $s2 > 0 ) {
...
if ($s1 > 0 or $s2 > 0) {
   if(strtotime($date_time) <= $date_1 or strtotime($date_time) <= $date_2){
        echo ' <button type="submit" class="btn btn-primary">ยืนยันการลงทะเบียน</button>';
     }
  }
echo ' <button type="button" class="btn btn-light" data-dismiss="modal">ปิด</button>
</div>';
